...
coxmod_Collagen <- coxph(Srv ~ Collagen+size+differentiation2+Nstage, data=training_Cohort)

total_Cohort$collagen_36 <- c(1- (summary(survfit(coxmod_collagen,newdata=total_Cohort), times=36)$surv))

dcr_total_collagen <- stdca(data=total_Cohort, outcome="group2", ttoutcome="DFS", timepoint=36, 
